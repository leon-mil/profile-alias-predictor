...
public sealed class ProfileAliasPredictor : ICommandPredictor
{
    private static readonly AliasDefinition[] AliasDefinitions =
        BuildAliasDefinitions();

    /// <summary>
    /// Builds the complete predictor command catalog.
    /// </summary>
    private static AliasDefinition[] BuildAliasDefinitions()
    {
        ...
        // =====================================================================
        // Residential Improvements deployment
        // =====================================================================

        AddResidentialImprovementsDeploymentAliases(aliases);


        // =====================================================================
        // Remaining profile commands
        // =====================================================================
    ...
    // =========================================================================
    // Residential Improvements deployment predictions
    // =========================================================================

    /// <summary>
    /// Adds every supported Residential Improvements deployment command.
    ///
    /// Workloads:
    ...
    ///
    /// Configuration modes:
    ///     no -Config = preserve destination application configuration
    ///     -Config    = deploy applicable configuration files
    ///
    /// Supported routes:
    ///     FEATURE -> LOCAL
    ///     FEATURE -> DEV
    ...
    ///     TEST    -> PROD
    /// </summary>
    private static void AddResidentialImprovementsDeploymentAliases(
        List<AliasDefinition> aliases)
    {
        ...
        var workloads = new (string Name, string Description)[]
        {
            (
                "COMPONENT_ESTIMATION",
                "COMPONENT_ESTIMATION"
        ...
        var routes = new (string Source, string Destination)[]
        {
            ("feature", "local"),
            ("feature", "dev"),
            ("feature", "test"),
        ...
        //
        // Every combination receives two variants:
        //
        // 1. Preserve destination configuration
        //
        ...
        // x 3 workloads
        // x 2 actions
        // x 2 configuration modes
        // = 120 deployment predictions
        // ---------------------------------------------------------------------
            ...
            foreach (var route in routes)
            {
                foreach (string action in actions)
                {
                    ...
                    string descriptionPrefix =
                        ...
                        $"{route.Source.ToUpperInvariant()} -> " +
                        $"{route.Destination.ToUpperInvariant()}";
                    ...
                    aliases.Add(
                        new AliasDefinition(
                            command,
                            $"{descriptionPrefix} - PRESERVE CONFIG"));
                    ...
                    aliases.Add(
                        new AliasDefinition(
                            command + " -Config",
                            $"{descriptionPrefix} - DEPLOY CONFIG"));
                }
            }
        }
    }
